...
{
   // TODO...
   //cout<<"in CmdParser::openDofile()"<<endl;//remove
   ifstream* tmp=new ifstream(string(dof).c_str());
   if(tmp->fail()){
      return false;
   }
...
CmdParser::closeDofile()
{
   ...
   delete _dofile ;
   _dofile=0;
   //cout<<"in CmdParser::closeDofile()"<<endl;//remove
...
CmdParser::execOneCmd()
{
   ...
   if (newCmd) {
      ...
      //cout<<"test1_execOneCmd: in inside"<<option<<endl;
      if (e != 0)
...
CmdParser::parseCmd(string& option)
{
   ...
   if(!(idx==string::npos))option=str.substr(idx,str.length());
   //option=str.substr(str.find_first_of(' '),str.length());//error
   
   // TODO...
   assert(str[0] != 0 && str[0] != ' ');
   commend=str.substr(0,str.find_first_of(' '));
   re=getCmd(commend);
   if(re==0){
      cerr<<"Illegal command!! ("<<commend<<")"<<endl;
...
CmdParser::getCmd(string str)
{
   ...
   //cout<<"parseCmd str:  "<<str<<endl;//mustremove
   for (unsigned int i=1;i<=str.length();i++){
      subs=str.substr(0,i);
     // cout<<"parseCmd sub : "<<subs<<endl;//mustremove
      transform(subs.begin(),subs.end(),subs.begin(),::toupper);
      iter=_cmdMap.find(subs);
      if (iter != _cmdMap.end()){
         string opt;
		   //cout << "Find the value is "<<subs<< endl;//mustremove
         re=iter->second;
         opt=re->getOptCmd();
         if(str.length()-i > opt.length())break;
         else{
            subs=str.substr(i,str.length());
            //cout<<"match: "<<subs<<endl;//mustremove
            if(subs.compare(opt.substr(0,subs.length()))==0){
              // cout<<"match !"<<endl;//mustremove
               return re;
            }
            break;
         }    
