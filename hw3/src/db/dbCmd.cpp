...
initDbCmd()
{
  ...
  // cout<<"initDbCmd: ok"<<endl;//mustremove
   return true;
}
...
DBAppendCmd::exec(const string& option)
{
   ...
   if(!lexOptions(option,tokens,2))return CMD_EXEC_ERROR;
   if((!dbjson)){
      cerr<<"Error: DB is not created yet!!";
      return CMD_EXEC_ERROR;
   }
   if(!isValidVarName(tokens[0])){
      cerr<<"Error: Illegal option!! ("<<tokens[0]<<")"<<endl;
      return CMD_EXEC_ERROR;
   }
   if(!myStr2Int(tokens[1],num)){
      cerr<<"Error: Illegal option!! ("<<tokens[1]<<")"<<endl;
      return CMD_EXEC_ERROR;
   }
   if(dbjson.add(DBJsonElem(tokens[0],num))){
      return CMD_EXEC_DONE;
   }else{
      cerr<<"Error: Element with key \""<<tokens[0]<<"\" already exists!!"<<endl;
       return CMD_EXEC_ERROR;
   }
}
...
{  
   ...
   if (!CmdExec::lexSingleOption(option, token))
      //cout<<"HelpCmd:exec : error"<<endl;//mustremove
      return CMD_EXEC_ERROR;
   if(!dbjson){
   ...
   if (token.size()) {
     //cout<<"DBPrintCmd::exec m"<<endl;//mustremove
     const DBJsonElem* dje;
     dje=dbjson.find(token);
     if(dje==nullptr){
     ...
     for (unsigned int i=0;i<dbjson.size();i++){
        if(dbjson[i].key().compare(token)==0){
           cout<<"{ "<<dbjson[i]<<" }"<<endl;
           return CMD_EXEC_DONE;
        }
     }
     */
     ...
     // cout<<"DBPrintCmd::exec s"<<endl;//mustremove
      cout<<dbjson;
      return CMD_EXEC_DONE;
   }
   //return CMD_EXEC_DONE;
