...
{
   if(nMachines==0)return;
   for(size_t s=0;s<nMachines;++s){
       TaskNode ins=TaskNode();
        //cout<<"Task node inserted: "<<ins<<endl;
        if(_taskHash.insert(ins)){
         _taskHeap.insert(ins);
         cout<<"Task node inserted: "<<ins<<endl;
        }else{
           --s;
        }
...
{
   // TODO...    //cout<<"Task node inserted: "<<ins<<endl;
   TaskNode ins=TaskNode(s,l);
   if(_taskHash.insert(ins)){
      _taskHeap.insert(ins);
      cout<<"Task node inserted: "<<ins<<endl;
      return true;
