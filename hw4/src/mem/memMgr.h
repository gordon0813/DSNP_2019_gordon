...
#define toSizeT(t)      (t -1+ SIZE_T - (t-1)%SIZE_T )  // TODO
//
...
class MemBlock
{
   ...
   bool getMem(size_t t, T*& ret) {
      // TODO  working
      if(t<=(_end-_ptr)/sizeof(char)){
         ret=(T*)(void*)this->_ptr;
         this->_ptr+=t;
        // cout<<t<<" "<<toSizeT(t)<<" "<<SIZE_T<< endl;
         return true;
      }else{
         return false;
      }
...
class MemRecycleList
{
   ...
   void reset() {
      ...
      if(_nextList!=nullptr){
         delete _nextList;
         //cout<<"==================reseting list: "<<_arrSize<<endl;
      };
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      while(_activeBlock->_nextBlock!=nullptr){
         tmpBlock=this->_activeBlock;
         _activeBlock=_activeBlock->_nextBlock;
         delete tmpBlock;
      }
      for(size_t i=0;i<R_SIZE;i++){
         _recycleList[i].reset();
         _recycleList[i]._first=nullptr;
         _recycleList[i]._nextList=nullptr;

         //cout<<"================list size: "<<_recycleList[i]._arrSize<<endl;
      }

      if(size_t(_activeBlock->_end-_activeBlock->_begin)!=b){
         delete _activeBlock;
   ...
   void  freeArr(T* p) {
      ...
      size_t n = 0;
      n=*((size_t*)(void*)p);
      //cout<<"================size in free array: "<<n<<endl;
      #ifdef MEM_DEBUG
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      size_t m = n % R_SIZE;
      MemRecycleList<T>* tmp=this->_recycleList+m;
      while(true){
         if(tmp->_arrSize==n){
            return tmp;
         }else if(tmp->_nextList!=nullptr){
            tmp = tmp->_nextList;
         }else{
            //cout<<"===============create: recyclelist: "<<n<<endl;
            tmp->_nextList=new MemRecycleList<T>(n);
   ...
   T* getMem(size_t t) {
      ...
      // TODO
      if(t>_blockSize){
         cerr<<"Requested memory ("<<t<<") is greater than block size("<<this->_blockSize<<"). Exception raised..."<<endl;
         throw bad_alloc();
         }//throw exception
      ...
      // TODO
         bool tf;
         tf=this->_activeBlock->getMem(t,ret);
         #ifdef MEM_DEBUG
         if(tf)cout << "Memory acquired from block... " << ret << endl;
         else cout<< "Memory acquired from block...fail ";
         #endif // MEM_DEBUG
         if(!tf){
         size_t rm_size=_activeBlock->getRemainSize();
         if(rm_size<toSizeT(sizeof(T)));
         else{
            size_t memleft=(rm_size -8 )/(sizeof(T));
        // cout<<"=======================left n blocks: "<<memleft<<endl;
            this->getMemRecycleList(memleft)->pushFront((T*)(void*)(_activeBlock->_ptr));
         }
         this->_activeBlock=new MemBlock<T>(_activeBlock,_activeBlock->_end-_activeBlock->_begin);
